...
Asistencias1::Asistencias1(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::Asistencias1)
{
    ui->setupUi(this);
    Mostrardatos();
}
...
//---------------------------------START DATE SHOW---------------------------------
void Asistencias1::Mostrardatos()
{
    QString consulta;
...
void Asistencias1::Mostrardatos1()
{
    QString consulta;
    consulta.append("SELECT * FROM Asistencias WHERE (AlumnoId="+ui->lineEditId->text()+") and (MateriaID=1)");
    QSqlQuery select;
...
void Asistencias1::Mostrardatos2()
{
    int fila = 0;
    ...
    ui->tableWidgetDatosa->setRowCount(0);
    QString consulta;
    consulta.append("SELECT *FROM Asistencias Where MateriaID=1");
    QSqlQuery consultar;
    consultar.prepare(consulta);
...
//---------------------------------START SECOND QUERY ------------------------------------
        _Idcurrent = ""+consultar.value(3).toString()+"";
            ...
            QString consulta90;
            consulta90.append("SELECT * FROM alumnos WHERE (MateriaID=1) and (AlumnoID="+_Idcurrent+")");
...
void Asistencias1::Obtenervalores1()
{
    QString consulta;
        consulta.append("SELECT * FROM alumnos WHERE (MateriaID=1) and (Fecha="+ui->lineEditFecha->text()+")");
        QSqlQuery consultar;
        consultar.prepare(consulta);
...
void Asistencias1::Obtenervalores()
{
    QString consulta;
        consulta.append("SELECT * FROM alumnos WHERE (MateriaID=1) and (AlumnoID="+ui->lineEditId->text()+")");
        QSqlQuery consultar;
        consultar.prepare(consulta);
...
//---------------------------------START IF EXIST---------------------------------
void Asistencias1::Idexiste()
{   _Id = "";
    QSqlQuery q("SELECT * FROM asistencias WHERE MateriaID=1;");
...
//---------------------------------START BUTONNS---------------------------------
void Asistencias1::on_pushButtonID_clicked()
{
    Idexiste();
    if (_Id == "NOTNULL"){
...
void Asistencias1::on_pushButtonFecha_clicked()
{
