...
Asistencias3::Asistencias3(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::Asistencias3)
{
    ui->setupUi(this);
    Mostrardatos();
}
...
//---------------------------------START DATE SHOW---------------------------------
void Asistencias3::Mostrardatos()
{
    QString consulta;
...
void Asistencias3::Mostrardatos1()
{
    QString consulta;
    consulta.append("SELECT * FROM Asistencias WHERE (AlumnoId="+ui->lineEditId->text()+") and (MateriaID=3)");
    QSqlQuery select;
...
void Asistencias3::Mostrardatos2()
{
    int fila = 0;
    ...
    ui->tableWidgetDatosa->setRowCount(0);
    QString consulta;
    consulta.append("SELECT *FROM Asistencias Where MateriaID=3");
    QSqlQuery consultar;
    consultar.prepare(consulta);
...
//---------------------------------START SECOND QUERY ------------------------------------
        _Idcurrent = ""+consultar.value(3).toString()+"";
            ...
            QString consulta90;
            consulta90.append("SELECT * FROM alumnos WHERE (MateriaID=3) and (AlumnoID="+_Idcurrent+")");
...
void Asistencias3::Obtenervalores1()
{
    QString consulta;
        consulta.append("SELECT * FROM alumnos WHERE (MateriaID=3) and (Fecha="+ui->lineEditFecha->text()+")");
        QSqlQuery consultar;
        consultar.prepare(consulta);
...
void Asistencias3::Obtenervalores()
{
    QString consulta;
        consulta.append("SELECT * FROM alumnos WHERE (MateriaID=3) and (AlumnoID="+ui->lineEditId->text()+")");
        QSqlQuery consultar;
        consultar.prepare(consulta);
...
//---------------------------------START IF EXIST---------------------------------
void Asistencias3::Idexiste()
{   _Id = "";
    QSqlQuery q("SELECT * FROM asistencias WHERE MateriaID=3;");
...
//---------------------------------START BUTONNS---------------------------------
void Asistencias3::on_pushButtonID_clicked()
{
    Idexiste();
    if (_Id == "NOTNULL"){
...
void Asistencias3::on_pushButtonFecha_clicked()
{
